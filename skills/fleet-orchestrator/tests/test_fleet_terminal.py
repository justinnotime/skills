"""Fleet terminal resolution never guesses a different server, session or database."""
...
def selection(tmp_path):
    ...
    profiles.mkdir()
    fleets = tmp_path / "fleets"
    (fleets / "primary").mkdir(parents=True)  # saved work of a session fleet
    config = tmp_path / "config.json"
    config.write_text(json.dumps({
        "schema": "fleet-runtime/v1",
        "fleets": {"profile_directory": str(profiles), "runtime_directory": str(fleets)},
        "runtime_dir": {"env": "NOTES_RUNTIME_DIR", "default": str(tmp_path / "machine")},
    }))
    env = {
...
def server_down(args, process_env):
    """Every server is stopped: session fleets resolve from saved work alone."""
    assert "TMUX" not in process_env and "TMUX_PANE" not in process_env
    return completed(code=1, stderr=f"no server running on /tmp/{args[1]}")


PRIMARY = {"name": "primary", "tmux_server": "primary-server", "primary_session": "primary"}


def test_session_fleet_resolves_from_saved_work_without_a_profile(selection, tmp_path):
    env, _, profiles = selection
    with mock.patch.object(profile, "_terminal_tmux", side_effect=server_down):
        assert profile.terminal_target("primary", env) == PRIMARY
        assert profile.public_view("primary", env)["profile_path"] == ""
        selected = profile.command_env("primary", env)
    assert selected["NW_FLEET"] == "primary"
    assert selected["NW_TMUX_SERVER"] == "primary-server"
    assert selected["DISPATCH_LEDGER_DB"] == str(
        tmp_path / "fleets/primary/state/fleet-orchestrator/dispatch-ledger.sqlite3")
    assert not (profiles / "primary.json").exists()


def test_selecting_a_fleet_replaces_any_inherited_selection(selection):
    env, _, _ = selection
    partial = {**env, "NW_FLEET": "alpha", "NW_TMUX_SERVER": "named-alpha",
               "NOTES_RUNTIME_DIR": "/wrong/runtime", "DISPATCH_LEDGER_DB": "/wrong/database",
               "AGENT_BUS_CFG": "/wrong/bus", "MATRIX_BUS_ROOM": "!stale:example.test"}
    with mock.patch.object(profile, "_terminal_tmux", side_effect=server_down):
        assert profile.terminal_target("primary", partial)["tmux_server"] == "primary-server"
        cleared = profile.command_env("primary", partial)
    assert "/wrong" not in json.dumps(cleared)
    assert "MATRIX_BUS_ROOM" not in cleared
    assert cleared["NOTES_RUNTIME_DIR"].endswith("/fleets/primary")


def test_inherited_manual_server_never_redirects_session_fleets(selection):
    env, _, _ = selection
    env.pop("NW_DEFAULT_TMUX_SERVER")
    env["NW_TMUX_SERVER"] = "manual-server"
    with mock.patch.object(profile, "_terminal_tmux", side_effect=server_down):
        assert profile.terminal_target("primary", env)["tmux_server"] == "default"


def test_selector_file_names_the_local_server_after_scrubbing_named_runtime(selection, tmp_path):
    env, _, _ = selection
    env.pop("NW_DEFAULT_TMUX_SERVER")
    selector = tmp_path / "machine/state/fleet-orchestrator/tmux-server"
    selector.parent.mkdir(parents=True)
    selector.write_text("original-server\n")
    named = profile.command_env("alpha", env)
    with mock.patch.object(profile, "_terminal_tmux", side_effect=server_down):
        assert profile.terminal_target("primary", named)["tmux_server"] == "original-server"
...
def test_terminal_profile_discovery_ignores_named_environment_derived_roots(selection):
    ...
    assert [row["name"] for row in rows] == ["alpha", "beta", "primary"]
    assert all(row["status"] != "invalid" for row in rows)
    assert profile.terminal_target("beta", inherited)["tmux_server"] == "named-beta"


@pytest.mark.parametrize("value", ["invalid name", "bad/server"])
def test_invalid_local_server_override_does_not_select_a_terminal(selection, value):
    env, _, _ = selection
    env["NW_DEFAULT_TMUX_SERVER"] = value
    with pytest.raises(profile.FleetProfileError):
        profile.terminal_target("primary", env)
...
def test_outside_tmux_requires_an_explicit_selection(selection):
    env, _, _ = selection
    assert profile.terminal_target(env={**env, "NW_FLEET": "alpha"})["name"] == "alpha"
    with mock.patch.object(profile, "_terminal_tmux", side_effect=server_down):
        assert profile.terminal_target(env={**env, "NW_FLEET": "primary"})["name"] == "primary"
    with pytest.raises(profile.FleetProfileError, match="no fleet selected.*orc fleet NAME"):
        profile.terminal_target(env=env)
    with pytest.raises(profile.FleetProfileError, match="does not exist.*orc fleet absent start"):
...
def test_actual_socket_and_primary_group_override_stale_fleet(selection, current_session, current_group):
    ...
    def tmux(args, process_env):
        ...
        assert args[:1] == ["-L"]
        assert "TMUX" not in process_env and "TMUX_PANE" not in process_env
        if DISCOVERY in args[-1]:
            return completed(code=1, stderr="no server running")
...
def test_plain_current_session_is_not_a_fleet(selection):
    env, _, _ = selection
    env.update(TMUX="/tmp/primary-server,4,0", TMUX_PANE="%5", NW_FLEET="alpha")

    def tmux(args, process_env):
        if args[0] == "display-message":
            return completed("/tmp/primary-server\t4\t%5\tscratch\t\n")
        if DISCOVERY in args[-1]:
            # The plain session has no history key and no saved work.
            return completed("scratch\t\t\t$3\nprimary\tprimary\tprimary\t$1\n")
        return completed(f"primary\tprimary\t/tmp/{args[1]}\nscratch\t\t/tmp/{args[1]}\n")

    with mock.patch.object(profile, "_terminal_tmux", side_effect=tmux):
        assert "scratch" not in profile.local_sessions(env)
        with pytest.raises(profile.FleetProfileError,
                           match="'scratch' is not a fleet.*orc fleet NAME start"):
            profile.terminal_target(env=env)
...
def test_stale_tmux_pair_means_outside_tmux(selection, answer):
    ...
    terminal is outside tmux, so only NW_FLEET applies, exactly as when the
    ...
    with mock.patch.object(profile, "_terminal_tmux", side_effect=tmux):
        ...
        assert profile.terminal_target(env={**env, **STALE_PAIR, "NW_FLEET": "primary"})["name"] == "primary"
        assert profile.terminal_target(env={**env, **STALE_PAIR, "NW_FLEET": "alpha"})["name"] == "alpha"
        with pytest.raises(profile.FleetProfileError, match="no fleet selected"):
...
def test_unverifiable_tmux_pair_is_never_probed(selection, pair):
    ...
    with mock.patch.object(profile, "_terminal_tmux", side_effect=tmux):
        ...
        assert profile.terminal_target(env={**env, **pair, "NW_FLEET": "primary"})["name"] == "primary"
        assert profile.terminal_target(env={**env, **pair, "NW_FLEET": "alpha"})["name"] == "alpha"
        with pytest.raises(profile.FleetProfileError, match="no fleet selected"):
            profile.terminal_target(env={**env, **pair})
...
def test_terminal_cli_ignores_stale_pair_from_a_daemon(selection, tmp_path):
    ...
    result = subprocess.run([sys.executable, "-B", str(SCRIPT), "terminal"],
                            env={**env, "NW_FLEET": "primary"}, text=True, capture_output=True, check=False)
    assert result.returncode == 0, result.stderr
    assert result.stdout == "primary\tprimary-server\tprimary\n"
    result = subprocess.run([sys.executable, "-B", str(SCRIPT), "terminal"],
    ...
    assert "no fleet selected" in result.stderr
...
def test_inventory_distinguishes_online_stopped_offline_and_missing_primary_without_mutation(selection, tmp_path):
    env, _, profiles = selection
    (tmp_path / "fleets/paused").mkdir()
    ...
    def tmux(args, process_env):
        ...
        if args[1] == "primary-server":
            if DISCOVERY in args[-1]:
                # A bound fleet, its grouped viewer, and a plain session without history or saved work.
                return completed("primary\tprimary\tprimary\t$1\n"
                                 "tview-a\tprimary\tprimary\t$2\n"
                                 "scratch\t\t\t$3\n")
            return completed("primary\tprimary\t/tmp/primary-server\n"
                             "tview-a\tprimary\t/tmp/primary-server\n"
                             "scratch\t\t/tmp/primary-server\n")
    ...
    assert [(row["name"], row["status"]) for row in rows] == [
        ("alpha", "offline"), ("beta", "missing-primary"), ("primary", "online"), ("paused", "stopped"),
    ]
    assert "scratch" not in json.dumps(rows)
    assert "orc fleet paused retire" in rows[3]["detail"]
    assert before == {p.name: p.read_bytes() for p in profiles.iterdir()}
    assert rows[2]["command"] == "tview -t primary"
    assert all(set(row) == {"name", "tmux_server", "primary_session", "status", "command", "detail"}
               for row in rows)
    assert not any("set-option" in args for args in seen), "listing must never bind a session"
...
def test_unavailable_probe_does_not_claim_absent_server_or_primary(selection, result, status):
    ...
    assert by_name["alpha"] == by_name["beta"] == status
    # A session fleet without a live server is paused, never "offline" or "missing".
    assert by_name["primary"] == ("stopped" if status == "offline" else status)
...
def test_selection_pins_terminal_observation_to_the_local_server_while_preserving_client_context(selection):
    env, _, _ = selection
    named = profile.command_env("alpha", env)
    ...
    assert cleared["DISPATCH_LEDGER_DB"].endswith("/fleets/primary/state/fleet-orchestrator/dispatch-ledger.sqlite3")
    code = (
...
def test_terminal_cli_emits_one_safe_tsv_record(selection, tmp_path):
    env, _, _ = selection
    env["TMUX_BIN"] = str(tmp_path / "no-tmux-here")
